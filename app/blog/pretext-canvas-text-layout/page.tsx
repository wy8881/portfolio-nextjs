    ...
    <div className="pt-16 md:pt-20 lg:pt-24">
      ...
      >
          <div className="prose prose-stone max-w-none space-y-10">
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">Text measurement</h2>
              ...
              <pre className="bg-artifact rounded-xl p-4 text-sm overflow-x-auto">
                <code className="text-primary">{`const prepared = prepareWithSegments(text, font)
const line = layoutNextLine(prepared, cursor, maxWidth)
// → { text, width, end: nextCursor }`}</code>
              </pre>
            </section>

            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">The layout loop</h2>
              <p className="text-secondary leading-relaxed mb-4">
                Text is laid out one horizontal band at a time, top to bottom. A band is a strip of space exactly
                one line-height tall at a specific vertical position. For each band:
              </p>
              <ol className="list-decimal list-inside space-y-2 text-secondary mb-4">
                <li>Ask every obstacle: &quot;what horizontal range do you block at this height?&quot; → returns an interval <code className="bg-artifact px-1 rounded">{`{ left, right }`}</code></li>
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">Obstacles</h2>
              <p className="text-secondary leading-relaxed mb-4">
                Two types of obstacles are supported, each requiring different math to compute their blocked interval per band.
              </p>
              <p className="text-secondary leading-relaxed mb-4">
                <strong className="text-primary">Circle obstacles</strong> — their footprint on a band varies by height. Wide in the middle, narrow near the top and bottom.
              ...
              <pre className="bg-artifact rounded-xl p-4 text-sm overflow-x-auto mb-4">
                <code className="text-primary">{`const minDy = /* distance from circle center to band */
const maxDx = Math.sqrt(r * r - minDy * minDy)
// blocked = { left: cx - maxDx - pad, right: cx + maxDx + pad }`}</code>
              </pre>
              <p className="text-secondary leading-relaxed">
                <strong className="text-primary">Rect obstacles</strong> — fixed rectangles that block the same width on every band they cover.
                No math needed beyond a simple overlap check.
              </p>
            </section>

            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">Moving objects</h2>
              <p className="text-secondary leading-relaxed mb-4">
              ...
              <pre className="bg-artifact rounded-xl p-4 text-sm overflow-x-auto">
                <code className="text-primary">{`const dist = Math.sqrt(dx * dx + dy * dy)
const step = Math.min(speed * dt, dist - stopDist)
dragon.x += (dx / dist) * step
dragon.y += (dy / dist) * step`}</code>
              </pre>
            </section>

            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">The animation loop</h2>
              <p className="text-secondary leading-relaxed">
                No <code className="bg-artifact px-1 rounded">setInterval</code> or <code className="bg-artifact px-1 rounded">while</code> loop.
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">DOM output</h2>
              <p className="text-secondary leading-relaxed">
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary mb-4">Why this approach</h2>
              <p className="text-secondary leading-relaxed mb-4">
                <strong className="text-primary">The old approach: CSS layout.</strong> The browser
                handles everything — line breaking, wrapping, flow. You write some markup, apply
                styles, and the engine figures out where text goes.
              </p>
              <p className="text-secondary leading-relaxed mb-4">
                <strong className="text-primary">The downside</strong> is that CSS layout runs once
                and is fixed relative to the document flow. You can float an image and text will
                wrap around it — but only rectangles, only static elements, and only within normal
                document flow. There is no way to make text reflow around a circle, around an
                element that moves, or around arbitrary geometry. The browser gives you no hook into
                the line-breaking process. And if you try to animate it by updating the DOM every
                frame, you destroy and reconstruct nodes on each tick — which is expensive and
                causes visible jank.
              </p>
              <p className="text-secondary leading-relaxed mb-4">
                <strong className="text-primary">Why destroy and reconstruct?</strong> When text
                changes position — say, an obstacle moves — the browser needs to recalculate where
                every line breaks. This process is called <em>reflow</em>. The browser measures
                each element&apos;s size and position, resolves how content wraps, then repaints the
                screen. Reflow is triggered any time the DOM structure or geometry changes. If you
                update text content or move elements around, the browser has to reflow the affected
                subtree. For animated layout — something changing every frame — the naive solution
                is to tear out the old DOM nodes and insert new ones with updated positions. But
                this forces a full reflow on every frame: the browser discards its cached layout,
                remeasures everything from scratch, and repaints. At 60fps, that cost adds up fast
                and produces visible jank.
              </p>
              <p className="text-secondary leading-relaxed">
                <strong className="text-primary">This approach</strong> takes ownership of the
                entire pipeline — measurement, line breaking, obstacle math, DOM writes — and runs
                it every frame. The cost is complexity. The benefit is that you can do things CSS
                simply cannot: text flowing around a curve, around overlapping shapes, around
                objects that move in real time. Any geometry becomes a valid obstacle. Any moving
