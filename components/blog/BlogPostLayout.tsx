      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex gap-2 mb-10 flex-wrap">
          {tags.map((tag) => (
            <span
              key={tag}
              className="text-xs px-2 py-1 rounded-full bg-artifact text-accent"
            >
              {tag}
            </span>
          ))}
        ...
        <Link
          href="/blog"
          className="text-sm text-secondary hover:text-accent transition-colors mt-12 inline-block"
        >
          ← Back to blog
        </Link>
      </div>
